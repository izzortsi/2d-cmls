...
using Random
using Agents, LinearAlgebra, InteractiveDynamics
using CairoMakie
...
function initialize_model(;
    ...
    space2d = ContinuousSpace((dims[1], dims[2]); periodic=true)
...
function agent_step!(particle, model)
    # Obtain the ids of neighbors within the particle's interaction radius
    
    ids = nearby_ids(particle, model, particle.interaction_radius)
    ids_array = [id for id in ids]
    particle.nb_size = length(ids_array)
...
model = initialize_model(; dims=(500, 500), params=params)
# %%

ids = nearby_ids(model.agents[1], model, model.agents[1].interaction_radius)
# %%
...
figure, = abm_plot(model)
figure
# %%

abm_video(
    "particles.mp4", model, agent_step!;
