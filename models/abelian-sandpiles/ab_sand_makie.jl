...
#seed = [1 2 0 1; 1 3 1 1; 1 2 3 1; 1 0 2 1]
...
scene = heatmap(outs[1], show_axis=false, colorrange=(0,4), colormap=[:black, :green, :purple, :yellow, :red], resolution=(854,480))
#scenef = heatmap(outs[end], show_axis=false, colorrange=(0,4), colormap=[:black, :green, :purple, :yellow, :red])#, resolution=(1280,720))
#cm = colorlegend(scene[end], raw=true, width=(4, 480), textsize=6)
#scene_final = vbox(scene, cm)

record(scene, "output$(time()).mkv", range(1, stop=niter), sleep=false, framerate=80, compression=51) do i
