...
scene = heatmap(outs[1], show_axis=false, colorrange=(0,4), colormap=:grays, resolution=(500,500))
...
record(scene, "output$(time()).mkv", range(1, stop=niter), framerate=80, sleep=false) do i
    heatmap!(outs[i], show_axis=false, colorrange=(0,4), colormap=:grays)
end
