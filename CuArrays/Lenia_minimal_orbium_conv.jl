...
L = Lenia(R, m, s, b, 1/5)
#%%
heatmap(L.K[((SIZE ÷2) -29):((SIZE ÷2) +29), ((SIZE ÷2) -29):((SIZE ÷2) +29)])
#%%
...
function record_run(fig, A, L; nframes = 600, fps=36)
    ...
    mkpath(opath)
    filename = replace("orbia_$(Dates.Time(Dates.now()))", ":" => "_")

    GLMakie.record(fig, opath * filename * ".mp4", 1:nframes; framerate = fps) do i
        supdate!(A, L)
        nA[] = A
        nU[] = L.U
        nG[] = L.G
...
function gpu_frames(A, L::Lenia; nframes = 300)
    L.K = L.K[((SIZE ÷2) -29):((SIZE ÷2) +29), ((SIZE ÷2) -29):((SIZE ÷2) +29)]
    kdim, = size(L.K)
    A = cu(A)
    L.K = cu(L.K)
    L.U = CUDA.zeros(SIZE, SIZE)
    state_seq = [A]
    conv = setup_convolution(SIZE)
    for i = 1:nframes
        A = state_seq[end]
        conv(SIZE, A, L.K, L.U, kdim)
        L.G = L.δ.(L.U)
        A = CUDA.clamp.(A .+ L.dt .* L.G, 0, 1)
        push!(state_seq, deepcopy(A))
    end
    return state_seq
end
...
function rec_run_gpu(;nframes=300, fps=30)

    
    frames = gpu_frames(A, L)
    host_frames = frames .|> Array
    A_node = Node(A)
    fig, hm = heatmap(A_node)
